...
function PremiumPackCard({ pack, index }: { pack: Pack; index: number }) {
  const [imageError, setImageError] = useState(false);

  return (
    <div className={`${pack.color} p-6 brutalist-border brutalist-shadow ${pack.rotation} ${index > 0 ? 'mt-8 md:mt-0' : ''} flex flex-col items-center text-center relative text-white group cursor-not-allowed hover:scale-105 transition-transform`}>
...
export default function PremiumPacks() {
  const packs = [
    ...
    },
    {
      ...
      image: '/nairobi_nights.jpg'
    }
  ];

  return (
    <section id='categories' className="mt-24 mb-16 relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {packs.map((pack, index) => (
          <PremiumPackCard key={index} pack={pack} index={index} />
        ))}
      </div>
    </section>
  );
